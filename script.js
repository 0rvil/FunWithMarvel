...
const movieListDetailed = [
  ...
  },
  {
    ...
    releasePosition: 21,
    description: " Set in the 1990s,'Captain Marvel' is an all new adventure from a perviously unseen period in the history of the Marvel Cinematic Universe that follows the journey of Carol Danvers as she becomes one of the universe's most powerful heroes. While a galactic war between two alien races reaches Earth, Danvers finds herself and a small cadre of allies at the center of the maelstrom. ",
    budget: "152,000,000",
    gross: "1,002,651,774",
    rottenTomatoes: "78%"
  },
  ...
  },
  {
    ...
    moviePoster:"images/avengersendgame.jpg",
...
function goHome() {
  ...
  let homeContent = `
    <div class="card">
      ...
      <br>
      <p>In theaters April 26th.</p>
    </div>
    <div class="card">
      <p>Spider-man: Far From Home<p>
      <img src="images/spiderfarfromhome.jpg">
      <br>
      <p>In theaters July 2nd.</p>
    </div>
  `
  $("#display-area").append(homeContent)
}

// Helper function: create movie cards
function createMovieCard(movie){
  ...
  <div class="card">
     ...
     <p> Descritption:${movie["description"]}</p>
     <p>RottenTomatoes: ${movie["rottenTomatoes"]}
  </div>
  `
}
...
function sortByMCUTimeline() {
  $("#display-area").empty();
  let mcu_timeline = movieListDetailed;
  mcu_timeline.sort((a,b) => a.timelinePosition-b.timelinePosition);
  // Create and append each movie
  for(var i = 0; i < mcu_timeline.length; i++){
    let movie = mcu_timeline[i];
    let timeline_movie = createMovieCard(movie);
    $("#display-area").append(timeline_movie);
  }
  // Shorter solution that gets rid of the loop,
  // but contains a bug where clicking away from the tab and returning produces underfined results
  // mcu_timeline.forEach(createMovieCard);
  // $("#display-area").append(mcu_timeline);
...
function sortByReleaseDate() {
  $("#display-area").empty();
  let release_date = movieListDetailed;
  release_date.sort((a,b) => a.releasePosition-b.releasePosition);
  // Create and append each movie
  for(var i = 0; i < release_date.length; i++){
    let movie = release_date[i];
    let release_movie = createMovieCard(movie);
    $("#display-area").append(release_movie);
  }
  // Shorter solution that gets rid of the loop,
  // but contains a bug where clicking away from the tab and returning produces underfined results
  // release_date.forEach(createMovieCard);
  // $("#display-area").append(release_date)
}
...
function showCredits() {
  ...
  let credits = `
    <div class="navPage">
      ...
      Click <a href="http://www.marvel.com">here</a> to visit the official Marvel website.
    </div>
  `
  $("#display-area").append(credits)
}

// // Helper function:  for creating movie cards for the alternate forEach solution
// function createMovieCard(movie, index, arg){
//   arg[index] =
//   `
//   <div class="card">
//      <h1>${movie["title"]}</h1>
